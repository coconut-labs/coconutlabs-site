...
export function validateSpec(spec) {
  ...
  // Two things draw in accent without naming an ink role, so both have to be
  // counted or a figure could quietly carry two accents: a stack's "you are
  // here" marker, and an ab figure's delta callout. For an ab figure the delta
  // IS the single quantity the figure exists to show, so it takes the accent
  // and neither panel may claim one.
  const hereMarks = (d) => (d && Number.isInteger(d.here) ? 1 : 0);
  const accents = isAb
    ? countAccents(spec.a.data) +
      countAccents(spec.b.data) +
      hereMarks(spec.a.data) +
      hereMarks(spec.b.data) +
      (spec.delta ? 1 : 0)
